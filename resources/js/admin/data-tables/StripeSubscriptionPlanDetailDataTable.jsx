...
const StripeSubscriptionPlanDetailDataTable = () => {
            ...
            <td>
                <div className={"d-flex justify-content-end align-content-center align-items-center"}>
                    {!!item.can && !!item.can.view && (
                        <a className="text-primary cursor-pointer" href={route('admin.stripeSubscriptionPlanDetail.show', {stripeSubscriptionPlanDetail: item.id}).toString()}>
                            View
                        </a>
                    )}
                </div>
    ...
    return (
        <React.Fragment>
            <Card>
                <Header title={"Stripe Subscription Plan Detail"} />
                <div className={"row m-0 mb-4 p-0 d-flex justify-content-lg-between"}>
                    <div className="col-12 col-md-7 col-lg-8 m-0 mb-4 mb-md-0 p-0 pl-md-2 order-md-2">
                        <div className={"d-flex justify-content-end align-items-center"}>
                            ...
                            />
                            {can('create', 'stripeSubscriptionPlanDetail') && (
                                <Button
                                    type={"href"}
                                    label={"Create Stripe Subscription Plan Detail"}
                                    options={{
                                        href: route('admin.stripeSubscriptionPlanDetail.create').toString(),
                    ...
                    <div className={"col-12 m-0 p-0"}>
                        <div className="data-table">
                            <AjaxDynamicDataTable
                                ref={dataTableRef}
                                apiUrl={route('web-api.admin.stripeSubscriptionPlanDetail.data-table').toString()}
                                loadingComponent={<LoadingIndicator />}
                                noDataComponent={<EmptyTable itemName="stripe subscription plan details" />}
...
export default StripeSubscriptionPlanDetailDataTable;

const target = document.getElementById('stripe-subscription-plan-detail-data-table');
if (target) {
    ReactDOM.render(<StripeSubscriptionPlanDetailDataTable />, target);
}
